...
class TBUtility:
    ...
    def get_value(expression, body=None, value_type="string", get_tag=False, expression_instead_none=False):
        #print("[TBUtility get_value]",expression,body,value_type,get_tag,expression_instead_none)
        ...
        #print("[TBUtility target_str]",p1,p2,target_str)
        if get_tag:
            return target_str
        full_value = None
        try:
            if isinstance(body, dict) and target_str.split()[0] in body:
                if value_type.lower() == "string":
                    ...
                    #print("[if isinstance(body, dict) and target_str.split()[0] in body:]")
                    full_value=int(body.get(target_str.split()[0]))
                else:
                    full_value = body.get(target_str.split()[0])
                #print("[TBUtility full_value]",full_value)
            elif isinstance(body, (dict, list)):
                try:
                    #print("[TBUtility]",target_str)
                    jsonpath_expression = parse(target_str)
                    jsonpath_match = jsonpath_expression.find(body)
                    if jsonpath_match:
                        full_value = jsonpath_match[0].value
                        if value_type.lower() == "double":
                            #print("[elif isinstance(body, (dict, list)):]")
