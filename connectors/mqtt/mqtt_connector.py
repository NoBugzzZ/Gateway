...
class MqttConnector(Connector, Thread):
    ...
    def server_side_rpc_handler(self, content):
        # print("[MqttConnector server_side_rpc_handler content]", content)
        for rpc_config in self.__server_side_rpc:
            # print("[MqttConnector server_side_rpc_handler rpc_config]", rpc_config)
            if search(rpc_config["deviceNameFilter"], content["device"]):
                topic = content["data"].get("topic")
                data_to_send = json.dumps(content["data"].get("params"))
                print("[MqttConnector server_side_rpc_handler topic data_to_send]", topic,data_to_send)
                try:
                    self._client.publish(topic, data_to_send)
                    self.__log.debug("Send RPC with no response request to topic: %s with data %s", topic, data_to_send)
                except Exception as e:
                    self.__log.exception(e)
