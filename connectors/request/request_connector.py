...
class RequestConnector(Connector, Thread):
    def __init__(self, gateway, config, connector_type):
        super().__init__()
        # print("[RequestConnector __init__]", config, connector_type)
        ...
        self.__headers={}
        if self.__config["security"]["type"] == "basic":
            self.__security = HTTPBasicAuth(self.__config["security"]["username"], self.__config["security"]["password"])
        elif self.__config["security"]["type"] == "bearer":
            self.__headers = {"Authorization":"Bearer "+self.__config["security"]["token"]}
        #print("[RequestConnector __init__ self.__security]", self.__security)
    ...
    def server_side_rpc_handler(self, content):
        #print("[RequestConnector server_side_rpc_handler]",content)
        try:
            for rpc_request in self.__rpc_requests:
                #print("[RequestConnector server_side_rpc_handler rpc_request]",rpc_request)
                if fullmatch(rpc_request["deviceNameFilter"], content["device"]):
                    converted_data = rpc_request["converter"].convert(rpc_request, content)
                    #("[RequestConnector server_side_rpc_handler converted_data]",converted_data)
                    response_queue = Queue(1)
                    request_dict = {"config": {**rpc_request,
                                               **converted_data},
                                    "request": request}
                    request_dict["config"].get("uplink_converter")
                    #print("[RequestConnector server_side_rpc_handler request_dict]",request_dict)
    ...
    def __send_request(self, request, converter_queue, logger):
        #print("[RequestConnector __send_request]", request, converter_queue, logger)
        url = ""
        try:
            ...
            params = {
                ...
                "data": json.dumps(request["config"].get("data", {}))
            }
            # print("[RequestConnector __send_request params]", params)
            logger.debug(url)
            if request["config"].get("httpHeaders") is not None:
                params["headers"].update(request["config"]["httpHeaders"])
            #print("[RequestConnector __send_request params]", params)
            logger.debug("Request to %s will be sent", url)
            response = request["request"](**params)
            if params.get("method")=="POST":
                return
            #print("[RequestConnector __send_request response]", response,response.ok)
            #response = request["request"](method=params["method"],url=params["url"],)
            if response and response.ok:
                if not converter_queue.full():
                    data_to_storage = [url, request["converter"]]
                    try:
                        data_to_storage.append(response.json())
                        #print("[RequestConnector response]",response.json())
    ...
    def __process_data(self):
        try:
            if not self.__convert_queue.empty():
                ...
                data_to_send = {}
                # print("[RequestConnector __process_data data]",data)
                if isinstance(data, list):
                    for data_item in data:
                        # print("[RequestConnector __process_data data_item]",data_item)
                    ...
                    # print("[RequestConnector __process_data =========")
